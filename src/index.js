...
const { logOutgoingRequest, logIncomingResponse, loggerLevels } = require("./logger-utils");
...
module.exports = {
	...
	settings: {
		...
		$secureSettings: ["httpClient.defaultOptions.logger"],
		...
		_streamRequest(url, opt, streamPayload) {
			if (opt.method == "GET") {
				return this._client(url, opt).on("response", response => {
					...
					this.logger[loggerLevels(response.statusCode)](logIncomingResponse(response));
				});
			}
